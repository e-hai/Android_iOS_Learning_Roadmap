 ...
 * 摒弃用专业名词解释专业名词的生硬说教，聚焦：
 * 历史烂摊子（痛点） ➔ 设计师怎么想（权衡与抽象） ➔ 代码怎么落（白话语法） ➔ 底层怎么抹平开销。
 */

export const KOTLIN_FEATURES_MARKDOWN = `### Kotlin 六大现代特性的语言设计哲学全景图

1. **泛型型变 (out / in)**
   - 比喻：一筐苹果到底能不能当成一筐水果送给别人？
   - 历史痛点：Java 中 \`List<Dog>\` 无法直接赋值给 \`List<Animal>\`，导致调用点必须繁琐地手写 \`? extends Animal\` / \`? super Dog\`（PECS 原则）。
   - 设计哲学：既然某些类只吐出数据或只接收数据，直接在“定义类”时一次性向编译器声明读写契约（声明处型变），全工程调用点自然转换，零通配符。
   - 典型场景：\`List<out E>\` 只读集合、\`Flow<out T>\` 数据流、\`Comparable<in T>\`。

2. **委托机制 (by)**
   - 比喻：专业的事交给专门的代理人跑腿，主类只挂名。
   - 历史痛点：到处充斥着大量毫无营养的 \`if (_cache == null)\` 懒加载重复逻辑，或为了增强 1 个方法不得不为装饰器手写 30 个转发方法的胶水代码。
   - 设计哲学：将“存取访问”与“接口转发”抽象为通用的协议约定（Convention），用 \`by\` 让编译器自动合成隐藏代理字段并转交调用。
   - 典型场景：\`by lazy\` 延迟初始化、\`by viewModels()\` 跨配置生命周期感知、\`by Delegates.observable\`。

3. **扩展 (fun / val)**
   - 比喻：不破坏人家的原装封装，也能随手在人家口袋里塞个好用的小工具。
   - 历史痛点：无法修改系统或第三方库源码，导致项目堆满主谓颠倒的垃圾桶类 \`ViewUtils.setGone(view)\`，且彻底摧毁了 IDE 的智能联想提示。
   - 设计哲学：允许在类名前缀声明方法，以主谓自然的点语法调用；编译期静态降级为普通静态方法，首个参数即为接收者对象。
   - 典型场景：Android KTX 核心库（如 \`View.visibleOrGone()\`、\`Context.toast()\`）。

4. **带接收者 Lambda (T.() -> R)**
   - 比喻：闭门造车——直接把屋子借给你，进门后你就是当前的宿主主人。
   - 历史痛点：多层级树形配置或构建器模式充斥着 \`builder.xxx\` 或 \`it.xxx\` 等无意义前缀，视觉凌乱且缺乏直观嵌套层级。
   - 设计哲学：在闭包执行时隐式借调目标对象的 \`this\` 上下文，无需任何中间变量前缀，奠定声明式 UI（Compose）与类型安全 DSL 根基。
   - 典型场景：\`T.apply { ... }\`、Jetpack Compose 视图层级声明、Gradle KTS 脚本。

5. **内联优化 (inline / noinline / crossinline)**
   - 比喻：替身代打与就地拆包装——高级抽象不给运行时交“中介费”。
   - 历史痛点：高阶函数在 JVM 底层每次都会偷偷创建匿名内部类对象，高频循环或高帧率绘制时频繁引发内存抖动与 GC 掉帧。
   - 设计哲学：零成本抽象（Zero-cost Abstraction）。编译期直接将高阶函数体与 Lambda 字节码平铺展开到调用点，抹平对象分配与调用栈开销。
   - 典型场景：高频集合操作符（\`map\` / \`filter\` / \`forEach\`）、锁守卫 \`synchronized\`、耗时测量。

6. **泛型具现化 (reified)**
   - 比喻：借东风——让内联展开把调用现场的真实类型直接“焊死”在字节码里。
   - 历史痛点：JVM 泛型运行时擦除导致无法执行 \`is T\` 检查和 \`T::class.java\`，被迫在每个函数额外传一个难看的 \`Class<T> clazz\`。
   - 设计哲学：借力 \`inline\` 将代码展开至调用现场，利用调用处已明确知晓具体类型的时机，直接将具体类型字节码指令硬编码写入。
      ...
      <div class="kt-visual-header">
        <div class="kt-visual-header-left">
          ...
          <p class="kt-visual-subtitle">
            语法特性绝不是单纯的炫技，而是设计师为了解决前人写代码时天天骂娘的痛点做出的优雅抽象与工程折中。
          </p>
        </div>
        ...
        <article class="kt-story-card border-purple">
          ...
          <div class="kt-sc-body-grid">
            <div class="kt-sc-box box-pain">
              <div class="kt-box-label label-pain">
                <span class="kt-emoji">💥</span> 当年的烂摊子（遇到什么痛点）
              </div>
              <p class="kt-box-text">
            ...
            <div class="kt-sc-box box-idea">
              <div class="kt-box-label label-idea">
                <span class="kt-emoji">💡</span> 设计师的解题思路（怎么抽象与权衡）
              </div>
              <p class="kt-box-text">
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（声明处一次搞定）</span>
            </div>
...
<span class="kt-c-k">val</span> fruitSource: <span class="kt-c-t">Source</span>&lt;<span class="kt-c-t">Fruit</span>&gt; = appleSource <span class="kt-c-m">// ✅ 自然赋值！全工程调用处再无需写任何 ? extends</span></code></pre>
          </div>

          <div class="kt-sc-footer-grid">
            <div class="kt-sc-foot-item">
              <span class="kt-foot-badge">⚙️ 底层怎么做</span>
              <span>纯编译器前端类型检查约束；字节码自动降级为 Java 通配符，<strong>零包装类、零运行时性能罚款</strong>。</span>
            </div>
            <div class="kt-sc-foot-item">
              <span class="kt-foot-badge">🎯 现实影子</span>
        ...
        <article class="kt-story-card border-green">
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（干掉成吨样板）</span>
            </div>
...
<span class="kt-c-k">class</span> <span class="kt-c-t">LoggedList</span>&lt;<span class="kt-c-t">T</span>&gt;(<span class="kt-c-k">private val</span> inner: <span class="kt-c-t">MutableList</span>&lt;<span class="kt-c-t">T</span>&gt;) : <span class="kt-c-t">MutableList</span>&lt;<span class="kt-c-t">T</span>&gt; <span class="kt-c-k">by</span> inner {
  <span class="kt-c-k">override fun</span> <span class="kt-c-f">add</span>(element: <span class="kt-c-t">T</span>): <span class="kt-c-t">Boolean</span> { <span class="kt-c-f">log</span>(element); <span class="kt-c-k">return</span> inner.<span class="kt-c-f">add</span>(element) }
        ...
        <article class="kt-story-card border-blue">
          ...
          <div class="kt-sc-body-grid">
            <div class="kt-sc-box box-pain">
              ...
              <p class="kt-box-text">
                ...
                为了省事，老项目充斥着大量垃圾桶类：<code>ViewUtils.gone(view)</code>、<code>StringUtils.isEmpty(str)</code>。这极为难受：<br>
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（类名前缀声明）</span>
            </div>
...
context.<span class="kt-c-f">toast</span>(<span class="kt-c-s">"屏幕宽度: \${context.screenW}px"</span>)</code></pre>
        ...
        <article class="kt-story-card border-amber">
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（形参声明 T.() -> Unit）</span>
            </div>
...
<span class="kt-c-m">// 调用端：如沐春风的声明式树形结构，0 个多余前缀</span>
<span class="kt-c-k">val</span> client = <span class="kt-c-f">setupClient</span> {
  ...
  <span class="kt-c-f">header</span>(<span class="kt-c-s">"Token"</span>, <span class="kt-c-s">"xyz"</span>)
        ...
        <article class="kt-story-card border-rose">
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（加个 inline 关键字）</span>
            </div>
...
<span class="kt-c-m">// 调用处编译后：代码就地平铺展开，0 对象分配，并支持从闭包中直接 return 外层函数！</span></code></pre>
        ...
        <article class="kt-story-card border-cyan">
          ...
          <div class="kt-sc-code-box">
            <div class="kt-code-header">
              <span class="kt-code-tab">🛠️ 怎么落成代码（inline &lt;reified T&gt;）</span>
            </div>
