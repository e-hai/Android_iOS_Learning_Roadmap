...
export function renderSidebar(
  ...
  sidebar.setAttribute('aria-label', i18n.t('sidebar.aria_label'));
  ...
  topItem.dataset.navTarget = docMode === 'roadmap' ? 'home' : 'all';
  topItem.setAttribute('aria-current', isTopActive ? 'page' : 'false');
  ...
  if (docMode === 'roadmap') {
    ...
    topItem.innerHTML = `
      ${iconSvg}
      <span class="sidebar-item-title" style="font-weight:700;">${deepDivePlatform === 'android' ? 'Android' : 'iOS'} ${i18n.t('sidebar.deepdive_overview')}</span>
    `;
  ...
  if (docMode === 'roadmap') {
    ...
    stagesSection.innerHTML = `<span class="sidebar-section-title main-title">${i18n.t('sidebar.stages')}</span>`;
    ...
    stages.forEach((stage) => {
      ...
      item.dataset.navTarget = stage.id;
    ...
    domainsSection.innerHTML = `<span class="sidebar-section-title main-title">${deepDivePlatform === 'android' ? 'Android' : 'iOS'} ${i18n.t('sidebar.knowledge_tree')}</span>`;
    ...
    deepDiveDomains.forEach((domain) => {
      ...
      domainHeader.innerHTML = `
        ...
        <span class="sidebar-domain-count">${mods ? mods.length : 0} ${i18n.t('sidebar.chapters')}</span>
      `;
      ...
      if (mods && mods.length > 0) {
        ...
        mods.forEach((mod, modIdx) => {
          ...
          modItem.dataset.navTarget = `${domain.id}:${modIdx}`;
          modItem.setAttribute('aria-current', isChildActive ? 'page' : 'false');
    ...
    if (docMode === 'roadmap') {
      switchBtn.innerHTML = `
        ...
        <div style="flex:1;overflow:hidden;">
          <div style="font-size:12px;font-weight:700;color:var(--color-ink);line-height:1.2;">${i18n.t('sidebar.to_deepdive')}</div>
          <div style="font-size:11px;color:var(--color-ink-muted);margin-top:2px;">${i18n.t('sidebar.to_deepdive_desc')} ➔</div>
        </div>
      `;
      ...
      switchBtn.innerHTML = `
        ...
        <div style="flex:1;overflow:hidden;">
          <div style="font-size:12px;font-weight:700;color:var(--color-ink);line-height:1.2;">${i18n.t('sidebar.to_roadmap')}</div>
          <div style="font-size:11px;color:var(--color-ink-muted);margin-top:2px;">${i18n.t('sidebar.to_roadmap_desc')} ➔</div>
        </div>
      `;
