...
export function renderSidebar(
  ...
  if (docMode === 'roadmap') {
    ...
    // 2. Single-Platform Deep Dive: 5 Industrial Domains Section
    ...
    deepDiveDomains.forEach((domain) => {
      ...
      if (mods && mods.length > 0) {
        subHtml = `<span class="sidebar-item-sub" style="font-size:11px;color:var(--color-ink-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block;max-width:170px;">${mods[0].tag}: ${mods[0].title.split('(')[0].trim()}</span>`;
      }
      ...
      item.addEventListener('click', () => onSelect(domain.id));
      domainsSection.appendChild(item);
    });
