...
#### 回到 viewModelScope 的设计真相

通过上述实验，我们可以彻底看清官方为什么要把 \`viewModelScope\` 绑定为 \`SupervisorJob\`：

- **如果 viewModelScope 采用普通 Job()（灾难后果）**：
  用户在页面上点击【更新头像】接口报错崩溃，即使你在最外层拦住了没闪退，**整个 ViewModel 的根 Job 也已经彻底脑死亡（进入 Cancelled 状态）**。
  从此以后，用户再去点击页面上的【刷新列表】、【点赞】、【提交订单】等**所有按钮全部失效，再也发不出任何网络请求！**
- **因为 viewModelScope 采用了 SupervisorJob()（保障页面可用）**：
  单个按钮点击（子协程）报错，**只会由该点击事件自身承担**，根 Job 依然保持健康活跃。用户接着点击其他按钮依然能正常发起新请求。

#### 两道职责分明的安全防线

| 防线层次 | 负责的机制 | 核心使命（保住什么？） |
| :--- | :--- | :--- |
| **第一道防线** | **\`viewModelScope\` 内置的 \`SupervisorJob\`** | **保住页面不死**：单个接口报错不摧毁整个 ViewModel 作用域，不误杀其他正在跑的请求 |
| **第二道防线** | **业务代码里的 \`try-catch / runCatching\`** | **保住 App 不崩**：消费具体的业务异常，切到错误状态（\`UiState.Error\`），防止抛给系统导致闪退 |`,
      },
      {
        tag: '内存模型',
